...
function CatSearch({
  ...
  function checkFilters(cat: Cat) {
    if (!cat) {
      return false;
    } // sometimes cat doesn't exist?
    const metNameFilter =
      searchName == ""
        ? true
        : cat.name.display.toLowerCase().includes(searchName.toLowerCase()); // True by default since name filter can be empty.
    const metStatusFilter = filters["status"][cat.status];
    const metExperienceFilter = filters["experience"][cat.experienceLevel];

    const metMentorFilter = cat.mentor
      ? selectedCats.includes(cat.mentor.ID)
      : false; // Valid if cat is apprentice of a selected cat.
    const metApprenticeFilter =
      cat.apprentices.find((c) => selectedCats.includes(c.ID)) != undefined; // Valid if cat has an apprentice in selected cats.
    const metMatesFilter =
      cat.mates.find((c) => selectedCats.includes(c.ID)) != undefined; // Valid if cat has a mate in selected cats.
  ...
  function appendRandom(numOfCats: number = 1, resetSelected: boolean = false) {
    ...
    const selectableCats = catsToSearch.filter(
      (cat) => !currentlySelected.includes(cat.ID) && checkFilters(cat),
    );
    ...
    for (
      ...
      i <
      Math.min(
        selectableCats.length,
        numOfCats,
        maxSelection - currentlySelected.length,
      );
      i++
    ) {
    ...
    <div className="cat-search-container">
      <div className="cat-search-filters">
        ...
        <details>
            ...
            {Object.keys(filters["status"]).map((name) => {
              return (
                <Checkbox
                  ...
                  onChange={() => toggleFilter("status", name)}
                />
              );
            })}
          </ul>
          <div className="button-row">
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("status", true)}
            >
              Select All
            </button>
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("status", false)}
            >
              Deselect All
            </button>
          </div>
        ...
        <details>
            ...
            {Object.keys(filters["experience"]).map((name) => {
              return (
                <Checkbox
                  key={name}
                  label={name}
                  checked={filters["experience"][name]}
                  onChange={() => toggleFilter("experience", name)}
                />
              );
            })}
          </ul>
          <div className="button-row">
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("experience", true)}
            >
              Select All
            </button>
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("experience", false)}
            >
              Deselect All
            </button>
          </div>
        ...
        <details>
          ...
          <ul>
            <Checkbox
              label="Mentors of selected cats"
              checked={filters["affiliation"]["apprentice"]}
              onChange={() => toggleFilter("affiliation", "apprentice")}
            />
            <Checkbox
              label="Apprentices of selected cats"
              checked={filters["affiliation"]["mentor"]}
              onChange={() => toggleFilter("affiliation", "mentor")}
            />
            <Checkbox
              label="Mates of selected cats"
              checked={filters["affiliation"]["mates"]}
              onChange={() => toggleFilter("affiliation", "mates")}
            />
          </ul>
          <div className="button-row">
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("affiliation", true)}
            >
              Select All
            </button>
            <button
              tabIndex={0}
              onClick={() => toggleAllFilters("affiliation", false)}
            >
              Deselect All
            </button>
          </div>
      ...
      <div className="cat-search-cats">
        <div className="cat-search-list">
          {filteredCats
            .slice(
              ...
              currentPage * catsPerPage + catsPerPage,
            )
            .map((cat) => {
              return (
                <Checkbox
                  ...
                  label={
                    <div>
                      ...
                      <div className="cat-search-select-status">
                        {cat.status}
                      </div>
                    </div>
                  }
        ...
        <div className="cat-search-page-controls button-row">
          ...
          <button tabIndex={0} onClick={() => appendRandom(maxSelection, true)}>
            Select Random Group
          </button>
          <button
            tabIndex={0}
